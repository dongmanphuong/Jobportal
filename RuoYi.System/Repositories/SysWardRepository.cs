...
{
    public class SysWardRepository : BaseRepository<SysWard, WardDto>
    {
        public SysWardRepository(ISqlSugarRepository<SysWard> sqlSugarRepository)
        {
            Repo = sqlSugarRepository;
        }

        public override ISugarQueryable<SysWard> Queryable(WardDto dto)
        {
        ...
        public override ISugarQueryable<WardDto> DtoQueryable(WardDto dto)
        {
            return Repo.AsQueryable()
                //.LeftJoin<SubTable>((t, s) => t.Id == s.Id)
                .WhereIF(!string.IsNullOrEmpty(dto.Code), (t) => t.Code == dto.Code)
                .WhereIF(!string.IsNullOrEmpty(dto.Name), (t) => t.Name == dto.Name)
                .Select((d) => new WardDto
                {
                    CreateBy = d.CreateBy,
        ...
        protected override async Task FillRelatedDataAsync(IEnumerable<WardDto> dtos)
        {
            await base.FillRelatedDataAsync(dtos);

        }
        public async Task<List<SysWard>> SelectListAsync(int districtId)
        {
            // Create an empty query DTO; no filtering will be applied if properties are null/empty.
            var query = new WardDto { parentid = districtId };
