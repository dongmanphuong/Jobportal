...
using RuoYi.Framework.Templates;
...
public sealed class LoggingMonitorAttribute : Attribute, IAsyncActionFilter, IAsyncPageFilter, IOrderedFilter
{
    ...
    private List<string> GenerateExcetpionInfomationTemplate(Utf8JsonWriter writer, Exception exception, bool isValidationException)
    {
        ...
        else
        {
            ...
            {
                $"━━━━━━━━━━━━━━━  业务异常 ━━━━━━━━━━━━━━━"
                , $"##业务码## {serviceException?.Code}"
                , $"##业务消息## {serviceException?.ErrorMessage}"
            });
            ...
            writer.WritePropertyName("validation");
            writer.WriteStartObject();
            writer.WriteString("errorCode", serviceException?.Code?.ToString());
            writer.WriteString("message", serviceException?.Message);
            writer.WriteEndObject();
        }
    ...
    private async Task MonitorAsync(MethodInfo actionMethod, IDictionary<string, object> parameterValues, FilterContext context, dynamic next)
    {
        ...
        if (exception == null)
        {
            ...
            if (validationMetadata != null)
            {
                ...
                exception = new ServiceException(error, validationMetadata.StatusCode ?? StatusCodes.Status400BadRequest)
                {
        ...
        // 判断是否是验证异常
        var isValidationException = exception is ServiceException serviceException && serviceException.ValidationException;

        var monitorItems = new List<string>()
        {
              $"##Controller name## {actionMethod.DeclaringType.Name}"
            , $"##Action name## {actionMethod.Name}"
            , $"##Display name## {displayNameAttribute?.DisplayName}"
            , $"##Routing information## [area]: {areaName}; [controller]: {controllerName}; [action]: {actionName}"
            , $"##Request method## {httpMethod}"
            , $"##Request address## {requestUrl}"
            , $"##Source address## {refererUrl}"
            , $"##Request source## {requestFrom}"
            , $"##Browser identifier## {userAgent}"
            , $"##Client regional language## {acceptLanguage}"
            , $"##Client IP address## {remoteIPv4}"
            , $"##Server IP address## {localIPv4}"
            , $"##Client connection ID## {traceId}"
            , $"##Service thread ID## #{threadId}"
            , $"##Execution time## {timeOperation.ElapsedMilliseconds}ms"
            ,"━━━━━━━━━━━━━━━ Cookies ━━━━━━━━━━━━━━━"
            , $"##Request end## {requestHeaderCookies}"
            , $"##Response end## {responseHeaderCookies}"
            ,"━━━━━━━━━━━━━━━ System information ━━━━━━━━━━━━━━"
            , $"##System name## {osDescription}"
            , $"##System architecture## {osArchitecture}"
            , $"##Basic framework## {basicFramework} v{basicFrameworkVersion}"
            , $"##.NET framework## {frameworkDescription}"
            ,"━━━━━━━━━━━━━━━━ Startup information ━━━━━━━━━━━━━━━"
            , $"##Runtime environment## {environment}"
            , $"##Startup assembly## {entryAssemblyName}"
            , $"##Process name## {processName}"
            , $"##Hosted program## {deployServer}"
        };
        ...
        // 获取最终写入日志消息格式
        var finalMessage = GetJsonBehavior(JsonBehavior, monitorMethod) == RuoYi.Framework.Logging.JsonBehavior.OnlyJson ? jsonString : monitorMessage;
